...
def analyse(file_path: str | Path, relative_to_initial: bool = True) -> Path:
    """Calculate per-year decrease percentage between adjacent trim deltas.

    Input CSV is expected to be generated by `write(...)`, with columns:
    `delta,<year1>,<year2>,...`.

    Output CSV format:
    - rows: years
    - columns: deltas (same order as source delta rows)
    - values:
      - when `relative_to_initial=True` (default):
        `((v_i - v_(i-1)) / v_0) * 100`
      - when `relative_to_initial=False`:
        `-((v_i - v_(i-1)) / v_(i-1)) * 100`

    For the first delta column (no previous delta), output empty string.

    The source CSV is not modified. A new file is written as
    `<source_stem>_decrease.csv` in the same directory.

    Args:
        file_path: Source CSV file path.
        relative_to_initial: Whether to divide adjacent delta difference by the
            initial value (`delta=0` column) for each year.

    Returns:
        Absolute path of generated decrease CSV file.
    ...
    if len(rows) < 2:
        raise ValueError("CSV must contain header and at least one delta row.")

    header: list[str] = rows[0]
    if len(header) < 2 or header[0] != "delta":
        raise ValueError("CSV header must be: delta,<year1>,<year2>,...")

    years: list[str] = header[1:]
    ...
    for row in rows[1:]:
        ...
        delta_text: str = row[0].strip()
        try:
            float(delta_text)
        except ValueError:
            # Ignore summary rows such as mean/std/rsd.
            continue

        values: list[float] = []
        for raw_value in row[1 : year_count + 1]:
            try:
                values.append(float(raw_value))
            except ValueError as exc:
                raise ValueError(f"Invalid IF value in CSV: {raw_value!r}") from exc

        deltas.append(delta_text)
        matrix.append(values)

    if len(deltas) < 2:
        raise ValueError("At least two delta rows are required for decrease analysis.")

    output_rows: list[list[str]] = []
    output_header: list[str] = ["year", *deltas]
    output_rows.append(output_header)

    for year_index, year_text in enumerate(years):
        row_out: list[str] = [year_text]
        for delta_index in range(len(deltas)):
            if delta_index == 0:
                row_out.append("0")
                continue

            previous_value: float = matrix[delta_index - 1][year_index]
            current_value: float = matrix[delta_index][year_index]
            if relative_to_initial:
                initial_value: float = matrix[0][year_index]
                ...
                if previous_value == 0.0:
                    ...
                    decrease_pct = float(-((current_value - previous_value) / previous_value) * 100.0)
            row_out.append(str(decrease_pct))
